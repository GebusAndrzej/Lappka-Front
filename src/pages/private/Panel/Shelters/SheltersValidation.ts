...
export const ShelterValidation = Yup.object({
    ...
    geoLocation: Yup.object({
        latitude: Yup.number().required('Wymagane').min(-90).max(90).typeError('Niepoprawny typ danych, proszę wprowadzić liczbę'),
        longitude: Yup.number().required('Wymagane').min(-180).max(180).typeError('Niepoprawny typ danych, proszę wprowadzić liczbę')})
});